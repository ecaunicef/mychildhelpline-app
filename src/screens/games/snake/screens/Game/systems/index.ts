    ...
    (entities: Entities, { touches, dispatch, events }: Object) => {
        ...
        if (useSwipes) {
            ...
            for (const { delta } of moveTouches) {
                ...
                if (
                    delta.pageY &&
                    Math.abs(delta.pageY) > Math.abs(delta.pageX)
                ) {
        ...
        if (head.nextMove === 0) {
            ...
            const isSnakeHitTheRightWall =
                head.position[0] + head.xspeed >= gridSize
            const isSnakeHitTheTopWall = head.position[1] + head.yspeed < 0
            const isSnakeHitTheBottomWall =
                head.position[1] + head.yspeed >= gridSize
            const isSnakeHitTheWallHorizontally =
                isSnakeHitTheLeftWall || isSnakeHitTheRightWall
            const isSnakeHitTheWallVertically =
                isSnakeHitTheTopWall || isSnakeHitTheBottomWall
